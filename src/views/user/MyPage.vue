...
import { ref } from 'vue'
import {
    Camera, UserMinus, CreditCard, CheckCircle2, MapPin,
    ArrowRight, Quote, Star, X, MessageSquareText
...
const rideHistoryList = [
    {
        id: 1, start: "강남역 2번 출구", dest: "판교역", date: "2024.03.24 (일)",
        cost: "4,500원", people: 4, isDone: true, time: "어제"
    },
    {
        id: 2, start: "합정역 7번 출구", dest: "서울역", date: "2024.03.21 (목)",
        cost: "3,200원", people: 2, isDone: true, time: "3일 전"
    }
]
...
<template>
    <div class="h-full flex gap-4 p-4 overflow-hidden relative">
        ...
        <div
            ...
            <div class="p-8 border-b border-slate-100 flex justify-between items-center bg-white/50">
                <div>
                    ...
                    <p class="text-sm text-slate-400 font-medium mt-1">나의 동승 기록과 프로필 정보를 관리하세요.</p>
                </div>
                    ...
                    <div class="col-span-12 lg:col-span-4 space-y-6 flex flex-col">
                        ...
                        <div class="bg-white rounded-[2.5rem] p-7 border border-slate-100 shadow-sm">
                            <div class="flex justify-between items-start mb-4">
                                <div>
                                    <span
                                        class="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest block mb-1">나의
                                        매너 등급</span>
                            ...
                            <div class="w-full h-3 bg-slate-100 rounded-full overflow-hidden mb-4">
                                <div class="bg-gradient-to-r from-indigo-500 to-purple-500 h-full rounded-full"
                                    style="width: 85%"></div>
                            </div>
                    ...
                    <div class="col-span-12 lg:col-span-8 space-y-6">
                        <div class="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm">
                            <div class="flex items-center justify-between mb-6">
                                ...
                                <RouterLink to="/editpayment">
                                    <button
                                        class="px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-xs font-bold hover:bg-indigo-100 transition-all">카드
                                        추가</button>
                                </RouterLink>
                            </div>
                        ...
                        <div
                            ...
                            <div class="flex border-b border-slate-50">
                                <button @click="switchTab('history')"
                                    ...
                                    :class="activeTab === 'history' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/30' : 'border-transparent text-slate-400 hover:text-slate-600'">
                                    최근 탑승 기록
                                </button>
                                <button @click="switchTab('reviews')"
                                    class="flex-1 py-6 text-sm font-bold border-b-2 transition-all"
                                    :class="activeTab === 'reviews' ? 'border-indigo-600 text-indigo-600 bg-indigo-50/30' : 'border-transparent text-slate-400 hover:text-slate-600'">
                                    받은 리뷰 <span class="ml-1 text-[10px] bg-slate-100 px-1.5 py-0.5 rounded-md">2</span>
                                </button>
                            </div>

                            <div v-if="activeTab === 'history'"
                                class="flex-1 overflow-y-auto custom-scroll p-6 space-y-4">
                                <div v-for="ride in rideHistoryList" :key="ride.id" @click="openRideDetail(ride.id)"
                                    ...
                                    <div class="flex-1">
                                        <div class="flex justify-between items-start mb-1">
                                            <p class="text-sm font-bold text-slate-800">{{ ride.start }} → {{ ride.dest
                                            }}</p>
                                            <span
                                        ...
                                        <div class="flex items-center gap-3">
                                            <p class="text-xs text-slate-400 font-medium">{{ ride.cost }} 결제 · 동승 {{
                                                ride.people }}명</p>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div
                                        ...
                                        <div class="flex items-center gap-3 mb-4">
                                            <img src="https://api.dicebear.com/7.x/avataaars/svg?seed=Luna"
                                        ...
                                        <p class="text-sm text-slate-600 leading-relaxed italic">"시간 약속을 정말 칼같이 지키셔서 너무
                                            좋았어요!"</p>
        ...
        <div v-if="isRideHistoryModalOpen"
            ...
            <div class="bg-white w-full max-w-lg rounded-[3rem] p-8 shadow-2xl" @click.stop>
                ...
                <p class="text-slate-600 mb-8">{{ currentHistory.start }}에서 {{ currentHistory.dest }}까지의 여정입니다.</p>
                <button @click="isRideHistoryModalOpen = false"
                    class="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold">닫기</button>
            </div>
        </div>
