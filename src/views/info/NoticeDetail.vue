...
<template>
    <div class="flex h-screen p-4 gap-4 bg-[#f8fafc] font-['Pretendard'] overflow-hidden text-left">
        ...
        <div class="flex-1 glass-panel rounded-[2.5rem] overflow-hidden flex flex-col">
            <div class="border-b border-slate-100 bg-white/50 w-full text-left">
                <div class="max-w-5xl mx-auto px-8 p-8 flex items-center gap-4">
                    <button @click="$router.back()" class="p-2 hover:bg-slate-100 rounded-full transition-colors text-slate-400">
                        <ArrowLeft class="w-6 h-6" />
                    </button>
                    <div>
                        <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">공지사항 상세</h1>
                        <p class="text-sm text-slate-400 font-medium mt-1">탈래말래의 새로운 소식을 확인하세요.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 overflow-y-auto custom-scroll p-8">
                <div class="max-w-5xl mx-auto">
                    ...
                    <div v-else-if="notice" class="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 space-y-8 text-left">
                        
                        <div class="space-y-4 border-b border-slate-50 pb-8">
                            <div class="flex gap-2">
                                <span class="bg-indigo-50 text-indigo-600 text-[10px] font-bold px-3 py-1.5 rounded-xl uppercase tracking-wider">
                                    {{ notice.tag }}
                                </span>
                            </div>
                            ...
                            <div class="flex items-center text-slate-400 text-sm gap-4">
                                <span class="flex items-center gap-1.5"><Calendar class="w-4 h-4" /> {{ notice.date }}</span>
                                <span class="flex items-center gap-1.5"><Eye class="w-4 h-4" /> 조회수 {{ notice.views.toLocaleString() }}</span>
                            </div>
                        </div>
