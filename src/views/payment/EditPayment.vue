    ...
    <div class="min-h-screen flex items-center justify-center p-4 bg-slate-100">
        <div
            ...
            <div
                ...
                <button @click="router.back()" class="p-1.5 hover:bg-slate-100 rounded-full transition-colors">
                ...
                <form @submit.prevent="handleSubmit" class="space-y-5">
                    <div class="space-y-1.5">
                        ...
                        <div class="grid grid-cols-4 gap-2">
                            <input v-model="cardNum.p1" @input="handleInput('p1', $event, 'p2')" type="text"
                                maxlength="4"
                                ...
                                placeholder="0000" />
                            <input v-model="cardNum.p2" @input="handleInput('p2', $event, 'p3')" type="password"
                                maxlength="4" id="p2"
                                class="p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-center font-mono outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 text-xs"
                                placeholder="****" />
                            <input v-model="cardNum.p3" @input="handleInput('p3', $event, 'p4')" type="password"
                                maxlength="4" id="p3"
                                class="p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-center font-mono outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 text-xs"
                                placeholder="****" />
                            <input v-model="cardNum.p4" @input="handleInput('p4', $event, null)" type="text"
                                maxlength="4" id="p4"
                                class="p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-center font-mono outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 text-xs"
                                placeholder="0000" />
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1.5">
                            ...
                            <input :value="expiry" @input="handleExpiry" type="text" maxlength="7"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input v-model="cvc" type="password" maxlength="3"
                                class="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-mono outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 text-xs"
                                placeholder="3자리" />
                        </div>
                    </div>
            ...
            <div class="px-6 py-6 bg-white border-t border-slate-50 flex gap-2.5">
                <button @click="router.back()"
                    class="flex-1 py-3 text-xs font-bold text-slate-400 hover:text-slate-600 transition-colors">취소</button>
                <button @click="handleSubmit"
                    class="flex-[2] py-3 bg-indigo-600 text-white text-xs font-bold rounded-xl shadow-lg hover:bg-indigo-700 transition-all">카드
                    등록</button>
            </div>
        </div>
    </div>
